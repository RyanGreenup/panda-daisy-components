...
function flexRender(Comp, props) {
	if (!Comp) return null;
	if (typeof Comp === "function") return createComponent$1(Comp, props);
	return Comp;
}
function createSolidTable(options) {
	const resolvedOptions = mergeProps$1({
		state: {},
		onStateChange: () => {},
		renderFallbackValue: null,
		mergeOptions: (defaultOptions, options$1) => {
			return mergeProps$1(defaultOptions, options$1);
		}
	}, options);
	...
	createComputed(() => {
		table$1.setOptions((prev) => {
			return mergeProps$1(prev, options, {
				state: mergeProps$1(state, options.state || {}),
